...
using AvailableIndexes = Microsoft.PackageGraph.Storage.Index.AvailableIndexes;
    ...
    public abstract class MicrosoftUpdatePackage : IPackage
    {
        ...
        public IEnumerable<IContentFile> Files
        {
            get
            {
                ...
                {
                    _Files = _MetadataSource.GetFiles(this._Id, new Content.UpdateFileFactory()).Cast<UpdateFile>().ToList();
                    _FilesLoaded = true;
                }
        ...
        internal static MicrosoftUpdatePackage FromStoredMetadataXml(Stream metadataStream, IMetadataSource metadataStore, string locale = null)
        {
            ...
            {
                createdUpdate._Files = metadataStore.GetFiles(createdUpdate._Id, new Content.UpdateFileFactory()).Cast<UpdateFile>().ToList();
